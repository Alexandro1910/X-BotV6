...
memberlimit = 0
ator = 'SUBSCRIBE'
namo = 'Alexandro Sihotang'
cr = '*BOT INI SUDAH TERVERIFIKASI*'
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["6285253528689@s.whatsapp.net","6285263628789@s.whatsapp.net"] 
/************************************/
           ...
           return false
            ...
            case 'bitly':
               ...
               case 'help': 
         	   ...
         	   case 'owner':
         	   case 'creator':
                  client.sendMessage(from, {displayname: "Lex", vcard: vcard}, MessageType.contact, { quoted: mek})
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
                ...
                addKoinUser('6285263628789@s.whatsapp.net', fee)
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'dah jadi '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, 'username mana ?', text, {quoted: mek})
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`yahh @${mentioned[0].split('@')[0]} jabatan admin kamu sudah dicopot🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('%ag target yang ingin ditendang!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`asek dapat makan otw kick @${mentioned[0].split('@')[0]} 🤙`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 welcome/left 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
					break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ Mengaktifkan 𝗳𝗶𝘁𝘂𝗿 𝘀𝗶𝗺𝗶 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ Menonaktifkan 𝗳𝗶𝘁𝘂𝗿 𝗻𝘀𝗳𝘄 𝗱𝗶 group 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
				break
                ...
                if (!isGroup) return reply(ind.groupo())
                if (!isGroupAdmins) return reply(ind.admin())
                if (args.length < 1) return reply('Boo :D')
                if (args[0] === 'enable') {
				...
				case 'clone':
			  	...
			  	case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ Menonaktifkan EVENT 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️*')
					} else {
						reply(ind.satukos())
					}
					break 
